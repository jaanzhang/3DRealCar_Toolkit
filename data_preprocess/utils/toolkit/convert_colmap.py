...
def colmap_process(colmap_command, source_path, mask_path, camera, use_gpu):
    ...
    if exit_code != 0:
        logging.error(
            f"[ COLMAP ] Feature extraction failed with code {exit_code}. Exiting.")
        exit(exit_code)
    ...
    if exit_code != 0:
        logging.error(
            f"[ COLMAP ] Feature matching failed with code {exit_code}. Exiting.")
        exit(exit_code)
    ...
    if exit_code != 0:
        logging.error(
            f"[ COLMAP ] Mapper failed with code {exit_code}. Exiting.")
        exit(exit_code)
...
def main():
    ...
    retried_times = 0
    logging.info(
        f'[ COLMAP ] Start processing images for colmap, remove exif info')
    for fn in tqdm(os.listdir(os.path.join(args.source_path, 'input'))):
        if os.path.splitext(fn)[-1] in ['.jpg', '.png']:
            img = load_image(os.path.join(
                args.source_path, 'input', fn), mode='pil')
    ...
    while True:
        ...
        if succ:
            ...
            os.system(
                f'ln -s {os.path.abspath(args.source_path)}/input/* {args.source_path}/images')
            logging.info('[ COLMAP ] Done')
            break
        retried_times += 1
        os.system(f'rm -rf {args.source_path}/distorted')
        logging.error(
            f'[ COLMAP ] Retried {retried_times} times, Colmap failed, start to rebuild ...')
